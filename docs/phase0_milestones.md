...
> **実装メモ**: MelSpectrogramTransform（100bins, hop320, 50Hz）の動作を検証済み。Audio I/O は load/save/resample/segment/pad_or_trim を実装。ロギングはrank-aware（rank 0のみコンソール出力）でファイル+コンソール併用。24テスト合格（mel: 10件, audio: 14件）。
