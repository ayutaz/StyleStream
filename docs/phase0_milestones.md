...
- [x] 上記ディレクトリ構造が作成され、全 `__init__.py` が配置されている
- [x] `pip install -e .`（`uv pip install -e .`）でパッケージとしてインストール可能
- [x] `import stylestream` および各サブモジュールの import がエラーなく成功する
- [ ] `scripts/` 配下のスクリプトがスタブとして存在し、`--help` で引数一覧を表示できる

> **実装メモ**: 9パッケージ（destylizer, stylizer, vocoder, data, utils, training, eval, inference, tests）を作成。全ディレクトリに `__init__.py` を配置済み。`uv pip install -e .` で正常にインストール・インポート可能。
...
- [x] `pyproject.toml` に全依存パッケージが定義されている
- [x] `uv sync` で仮想環境が構築できる
- [x] `uv sync --extra train --extra eval --extra dev` で全依存を含む環境が構築できる
- [x] `python -c "import torch; print(torch.cuda.is_available())"` が `True` を返す（GPU環境）
- [x] `python -c "import transformers, accelerate, einops, torchaudio"` がエラーなく成功する
- [x] `.python-version` が PyTorch と互換性のあるバージョンに設定されている

> **実装メモ**: pyproject.toml にコア依存12個 + オプショナル3グループを定義。Python 3.13 から 3.12 に変更（PyTorch互換性のため）。主要依存: torch 2.10, torchaudio 2.10, transformers 5.3, accelerate。`uv sync` で正常動作確認済み。
...
- [x] 全設定ファイル（YAML）が `configs/` 配下に作成されている
- [x] `stylestream/config.py` に structured config の dataclass が定義されている
- [x] Hydra で設定をロードし、CLI からオーバーライドできる（例: `python scripts/train_destylizer.py destylizer.fsq.levels=[7,5,5] training.batch_size=16`）
- [x] 設定値が論文セクション10.8のパラメータ一覧と一致している
- [ ] 不正な設定値（型不一致、範囲外）に対してバリデーションエラーが発生する
- [x] `tests/test_config.py` で設定のロード・オーバーライド・バリデーションのテストが通る

> **実装メモ**: 9個のYAML設定ファイルを `configs/` 配下に作成。`stylestream/config.py` に18個のdataclassを定義。Hydra CLIオーバーライド動作確認済み。論文セクション10.8のパラメータを正確に反映。
...
- [x] `MelSpectrogramTransform` が `(batch, waveform_length)` 入力に対して `(batch, 100, T)` を返す
- [x] 50Hz であることを検証: `T == waveform_length // 320` が成立する
- [x] `load_audio` が WAV/FLAC/MP3 を読み込み、16kHz モノラルの `torch.Tensor` を返す
- [x] `save_audio` → `load_audio` のラウンドトリップでデータが保持される
- [x] メルスペクトログラムが Vocos の公式実装と同一パラメータで計算される
- [x] `tests/test_mel.py` でメル計算の形状・値の正当性テストが通る
- [x] `tests/test_audio.py` でオーディオI/Oのラウンドトリップテストが通る
- [x] ロガーが設定通りに動作し、分散学習時に rank 0 のみコンソール出力される

> **実装メモ**: MelSpectrogramTransform（100bins, hop320, 50Hz）の動作を検証済み。Audio I/O は load/save/resample/segment/pad_or_trim を実装。ロギングはrank-aware（rank 0のみコンソール出力）でファイル+コンソール併用。38テスト合格（mel: 24件, audio: 14件）。
...
- [x] `BaseTrainer` を継承したダミートレーナーで、単一GPU での学習ループが動作する
- [x] `accelerate launch --num_processes=2` で2GPU DDP 学習が動作する
- [x] チェックポイントの保存・読込・学習再開が正常に動作する
- [x] コサインアニーリングスケジューラが論文の設定（ウォームアップ + コサイン減衰）通りに動作する
- [ ] wandb にログが記録される（wandb がインストールされている場合）
- [ ] tensorboard にイベントファイルが出力される
- [x] gradient accumulation により、少数GPU でも大バッチサイズを模擬できる
- [x] `outputs/` ディレクトリに期待通りの構造でチェックポイントと設定が保存される

> **実装メモ**: BaseTrainer を accelerate ベースで実装し、DDP/FSDP 切替に対応。CosineAnnealingWarmup スケジューラは warmup→peak→cosine decay の動作を検証済み。CheckpointManager は safetensors 形式でモデル重みを保存。
...
- [x] `ModelRegistry` に全7モデルが登録されている
- [x] `scripts/download_models.py --stage train` で学習用モデル（HuBERT, WavLM, Vocos）がダウンロードされる
- [x] `scripts/download_models.py --stage eval` で評価用モデル（Whisper, Resemblyzer, accent-id, emotion2vec）がダウンロードされる
- [x] `load_hubert(device, layer=18)` が HuBERT の第18層隠れ状態を抽出する関数を返す
- [x] `load_wavlm(device)` が凍結済み WavLM モデルを返す
- [ ] `HF_HUB_OFFLINE=1` 設定時にキャッシュ済みモデルがロードできる
- [x] `verify_cache()` が未ダウンロードモデルを検出してレポートする
- [ ] ダウンロード済みモデルのリビジョン（コミットハッシュ）が設定ファイルで管理されている

> **実装メモ**: ModelRegistry に7モデル登録済み（HuBERT, WavLM, Vocos, Whisper, Resemblyzer, accent-ECAPA, emotion2vec）。`download_models.py` は `--list`, `--stage`, `--verify` オプションに対応。
