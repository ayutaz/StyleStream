...
Usage:
    uv run python scripts/train_stylizer.py \\
        --config configs/stylizer/offline.yaml

    uv run python scripts/train_stylizer.py \\
        --config configs/stylizer/offline.yaml \\
        --resume outputs/stylizer/checkpoints/step_10000
...
def _load_config(config_path: str | None, overrides: list[str]):
    ...
    if config_path is not None:
        ...
        base = OmegaConf.create({
            ...
            "data": {
                ...
                "content_features_dir": "data/processed/content_features",
    ...
    if not hasattr(base, "log_dir"):
        base.log_dir = str(Path(base.output_dir) / "stylizer" / "logs")
    ...
    if hasattr(base, "data"):
        ...
        if not hasattr(base.data, "content_features_dir"):
            base.data.content_features_dir = "data/processed/content_features"
...
def main() -> None:
    ...
    try:
        from omegaconf import OmegaConf
        print("=" * 60)
        print("Stylizer Training Configuration")
        print("=" * 60)
